import Image from 'next/image';
import { Reveal } from '@/components/site/Reveal';
import { ContactForm } from '@/components/site/ContactForm';
import {
  BUSINESS_HOURS,
  INSTAGRAM_HANDLE,
  INSTAGRAM_URL,
  LOCATION,
  WHATSAPP_PHONE,
  WHATSAPP_URL_RECANTO,
} from '@/lib/site-constants';

export const metadata = { title: 'Contato' };

export default function ContatoPage() {
  return (
    <main>
      <section style={{ paddingTop: 160, paddingBottom: 0 }}>
        <div className="wrap contact" style={{ padding: 0 }}>
          <Reveal>
            <div className="eyebrow">Contato</div>
            <h1>
              Fale com a <em>Construtora Fox</em>
            </h1>
            <p className="contact-lead">
              Nossa equipe está pronta para atender você e apresentar as melhores oportunidades
              disponíveis.
            </p>

            <a
              href={WHATSAPP_URL_RECANTO}
              className="whatsapp-card"
              target="_blank"
              rel="noopener noreferrer"
            >
              <div className="whatsapp-icon">
                <svg viewBox="0 0 24 24">
                  <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51l-.57-.01c-.198 0-.52.074-.792.372s-1.04 1.016-1.04 2.479 1.065 2.876 1.213 3.074c.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z" />
                </svg>
              </div>
              <div className="meta">
                <h3>Conversar no WhatsApp</h3>
                <p>{WHATSAPP_PHONE}</p>
              </div>
              <div className="arrow-circle">→</div>
            </a>

            <a
              href={INSTAGRAM_URL}
              className="whatsapp-card"
              target="_blank"
              rel="noopener noreferrer"
            >
              <div className="whatsapp-icon">
                <svg viewBox="0 0 24 24" style={{ width: 24, height: 24, display: 'block' }}>
                  <rect x="3" y="3" width="18" height="18" rx="5" ry="5" fill="none" stroke="#fff" strokeWidth="2" />
                  <circle cx="12" cy="12" r="4" fill="none" stroke="#fff" strokeWidth="2" />
                  <circle cx="17.5" cy="6.5" r="1.2" fill="#fff" stroke="none" />
                </svg>
              </div>
              <div className="meta">
                <h3>Acompanhe no Instagram</h3>
                <p>{INSTAGRAM_HANDLE}</p>
              </div>
              <div className="arrow-circle">→</div>
            </a>

            <div className="contact-info">
              <div className="contact-info-item">
                <h4>Atendimento</h4>
                <p>{BUSINESS_HOURS}</p>
              </div>
              <div className="contact-info-item">
                <h4>Localização</h4>
                <p>{LOCATION}</p>
              </div>
            </div>

            <div className="contact-agent">
              <Image
                src="/site/images/contact-agent.jpg"
                alt="Equipe Fox"
                width={80}
                height={80}
              />
              <div>
                <h4>Equipe Fox</h4>
                <p>Atendimento personalizado</p>
              </div>
            </div>
          </Reveal>

          <Reveal delay={1}>
            <ContactForm />
          </Reveal>
        </div>
      </section>
      <div style={{ height: 140 }} />
    </main>
  );
}
